...
Transform::Transform(const glm::vec3& pos, const glm::quat& q, const glm::vec3& scale) : 
	m_translation(pos), m_modelMat(glm::mat4(1)), m_scale(scale), m_parent(nullptr), m_quaternion(q)
{
...
Transform::Transform(const glm::vec3 &translation, const glm::vec3 &eulerAngles, const glm::vec3 &scale) 
	: m_parent(nullptr), m_scale(scale), m_eulerAngles(eulerAngles), m_translation(translation)
{
...
glm::vec3 Transform::getTranslationWorld()
{
	return GetTranslationFromModel(m_modelMat_world);
}

glm::quat Transform::getRotationWorld()
{
	return GetQuatFromModel(m_modelMat_world);
}

glm::mat4 Transform::getRotationMatWorld()
{
	return glm::toMat4(GetQuatFromModel(m_modelMat_world));
}
