...
void ResourceManager::RemoveObjectFromLast()
{
	if (m_obj_vec.size() > 0)
	{
		...
		*/
		...
		renderer->RemoveObjectMapCache(key_shader, object);
...
void ResourceManager::AddGameObject(std::shared_ptr<GameObject>& object)
{
	if (object->hasMesh())
	{
		...
		GLFWApp::getInstance()->getRenderer()->SetShaderObjectMap(mesh_shader, object);
	}
...
void ResourceManager::AddSimCharacter(std::shared_ptr<SimCharacter> sim_character)
{
	m_sim_character_vec.push_back(sim_character);
}
